...
#plot clustering
par(mfrow=c(1,1))
clus=hclust(dist(w.arr),method="ward")
plot(clus)
(cls <- identify(clus))
nclus=length(cls)
ju=rep(NA,nc)
...
par(mfrow=c(1,1))
map('state');points(coord$lon,coord$lat,pch=19,cex=2,col=ju)
...
jj=rep(1,nc)
jj[ju==i]=19
jc=rep(0,nc);jc[cls[[i]]]=pcw$loadings[,1]


map('state');

points(coord$lon,coord$lat,pch=jj,cex=1,col=color.scale(jc,c(1,0.5,0),c(0,0.5,0),c(0,0,1),color.spec="rgb"))
