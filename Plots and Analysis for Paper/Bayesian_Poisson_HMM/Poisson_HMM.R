...
stan_data <- list(N = length(ann_count),
                  K = 3,
                  y = Count_prox$Count)
hmm_fit <- stan("Poisson_HMM.stan", data = stan_data, iter = 20e3, warmup = 10e3, chains = 6, control = list(adapt_delta = 0.99, max_treedepth = 12))
...
write.table(lambda, "Emission_Parameters.txt", sep = " ")
...
for (i in 1:nrow(samples)) {
  psi_seq <- sapply(z_star[i,], function(x){lambda[i,x]})
