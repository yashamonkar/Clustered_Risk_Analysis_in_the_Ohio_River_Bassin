...
for(i in 1:num_sites){
  ...
  discharge <- renameNWISColumns(discharge)
  discharge$Year <- format(as.Date(discharge$Date, format="%Y-%m-%d"),"%Y")
  discharge <-  discharge %>% group_by(Year) %>% summarise(Ann_Max = max(Flow))
  Ann_Max_Streamflow[,i] <- discharge$Ann_Max
}
Ann_Max_Streamflow$Year <- Dataset_Years

write.table(final_sites, 'data/Site_Information.txt', sep=" ")
write.table(Ann_Max_Streamflow, 'data/Annual_Maximum_Streamflows.txt', sep=" ")
