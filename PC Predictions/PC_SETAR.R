...
#SETAR
embd <- k
selections <- selectSETAR(x, m=embd, thDelay=0:(embd-1), plot = FALSE)
...
for(j in 1:N_Sims) {
  ...
  sims_dens <- density(sims.setar$serie)
  }
...
embd_1st <- 4
selections_1st <- selectSETAR(x_1st, m=embd_1st, thDelay=0:(embd_1st-1), plot = FALSE)
...
MH[1:length(mh)] <- mh
B_1st <- c(ML,MH)
thDelay_1st <- selections_1st$firstBests[1]

#Plotting the density distribution. 
og_density <- density(x_1st)
...
embd_2nd <- 2
selections_2nd <- selectSETAR(x_2nd, m=embd_2nd, thDelay=0:(embd_2nd-1), plot = FALSE)
...
MH[1:length(mh)] <- mh
B_2nd <- c(ML,MH)
thDelay_2nd <- selections_2nd$firstBests[1]

#Plotting the density distribution. 
og_density <- density(x_2nd)
consolid_sims <- as.list(1)
consolid_points <- as.list(1)
for(j in 1:N_Sims) {
...
x_3rd <- scale(x_3rd)
embd_3rd <- 1
selections_3rd <- selectSETAR(x_3rd, m=embd_3rd, thDelay=0:(embd_3rd-1), plot = FALSE)
...
MH[1:length(mh)] <- mh
B_3rd <- c(ML,MH)
thDelay_3rd <- selections_3rd$firstBests[1]

#Plotting the density distribution. 
og_density <- density(x_3rd)
consolid_sims <- as.list(1)
consolid_points <- as.list(1)
