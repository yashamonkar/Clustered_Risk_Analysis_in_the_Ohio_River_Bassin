---
title: "Experiment:- Significance Level of Red Noise"
author: "Yash Amonkar"
date: "July 18, 2019"
...
#NUmber Significane Levels
Sig_levels <- seq(from = 0.55, to = 0.95, by = 0.05)

#DataSets to Store Information across the PC's
LTM_SKILL <- matrix(NA, nrow = length(Sig_levels), ncol = 1)
AR_SKILL <- matrix(NA, nrow = length(Sig_levels), ncol = 1)



#Master Loop
for(sig in 1:length(Sig_levels)) {
  print(sig)
  
input_data_pca <- prcomp(training_set, scale = TRUE)
var <- cumsum(input_data_pca$sdev^2)
npcs <- 1 #NUmber of Selected PCs
...
for(i in 1:ncol(prin_comp)) {
  p <- prin_comp[,i]
  wlt <- wavelet(p)
  C_r <- CI(Sig_levels[sig],p,'r')
  C_w <- CI(Sig_levels[sig],p,'w')
...
for(i in 1:nrow(ar_raw)) {
  ...
  AR_SKILL[sig,1] <- sum(ar_skill)
  LTM_SKILL[sig,1] <- sum(ltm_skill)
...
plot(Sig_levels,AR_SKILL/(ncol(input_data)*predict_ahead),type='l',
     xlab = "Red Noise Significance Level", 
...
plot(Sig_levels,LTM_SKILL/(ncol(input_data)*predict_ahead),type='l',
     xlab = "Red Noise Significance Level", 
     ylab = "Fraction of predictions",
