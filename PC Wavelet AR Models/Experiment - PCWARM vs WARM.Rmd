---
title: "Experiment:- PCWARM vs WARM Models"
author: "Yash Amonkar"
date: "July 18, 2019"
...
The point of this experiment is that the red noise significance level is used to divide between signal and noise. This experiment is to check the effect of level of significance in getting the data right. 

###Document can take any data. 
...
```{r}
input_data <- read.table("data/Simulated Series.txt", sep=" ",header=TRUE)
...
LTM_PCWARM <- matrix(NA, nrow = predict_ahead, ncol = 1)
AR_PCWARM <- matrix(NA, nrow = predict_ahead, ncol = 1)

  
input_data_pca <- prcomp(training_set, scale = TRUE)
var <- cumsum(input_data_pca$sdev^2)
npcs <- 2 #NUmber of Selected PCs
...
WARM_Predictions <- tot_pc_pred #These are the predictions
nComp = ncol(WARM_Predictions) 
WARM_Predictions =  WARM_Predictions %*% t(input_data_pca$rotation[,1:nComp])
for(i in 1:ncol(WARM_Predictions)) {   
  WARM_Predictions[,i] <- scale(WARM_Predictions[,i], center = FALSE , scale=1/input_data_pca$scale[i]) }

for(i in 1:ncol(WARM_Predictions)) {   
  WARM_Predictions[,i] <- scale(WARM_Predictions[,i], center = -1 * input_data_pca$center[i], scale=FALSE)
}

#Computing Site - Specific MSE
True_Values <- testing_set
site_MSE <- bias <- rep(NA,ncol(WARM_Predictions))
for(i in 1:ncol(WARM_Predictions)) {
  site_MSE[i] <- mean((WARM_Predictions[,i]-True_Values[,i])^2)
  bias[i] <- sign(sum(WARM_Predictions[,i]-True_Values[,i]))
}
...
#########Long Term Mean####################
All_Predictions <- WARM_Predictions
...
for(i in 1:nrow(ar_raw)) {
  ...
  AR_PCWARM <- rowSums(ar_skill)
  LTM_PCWARM <- rowSums(ltm_skill)
############################################################
...
#DataSets to Store Information across the PC's
LTM_WARM <- matrix(NA, nrow = predict_ahead, ncol = ncol(training_set))
AR_WARM <- matrix(NA, nrow = predict_ahead, ncol = ncol(training_set))

#Detrending the Data
detrended <- matrix(NA, ncol = ncol(training_set), nrow = nrow(training_set))
trend_coeff <- matrix(NA, ncol = 2, nrow = ncol(training_set))
par(mfrow=c(3,2))
par(mar = c(4, 1, 1.5, 1))
...
for(i in 1:ncol(training_set)) {
  ...
  C_r <- CI(0.9,p,'r')
  C_w <- CI(0.9,p,'w')
  plot(wlt$period,wlt$p.avg,xlim=c(0,75),main=paste0("GW Spectrum for PC ", i),xlab="Period",ylab="Variance")
...
########Fitting ARMA Models#############
signals <- list(NA)
tot_pc_pred <- matrix(NA,nrow=predict_ahead,ncol=ncol(training_set))
for(i in 1:ncol(training_set)) {
...
```{r}
ar_WARM <- rowSums(AR_WARM)
ltm_WARM <- rowSums(LTM_WARM)

#Plotting the AR
plot(1:predict_ahead, ar_WARM/(ncol(training_set)),
     xlab = "Prediction Horizon", 
     ylab = "Fraction of Predictions - Correct",
     main = "WARM and PCWARM against AR",
     ylim = c(0,1), 
     type='l'
     )
lines(1:predict_ahead, AR_PCWARM/(ncol(training_set)),
      col = 'red')
legend("topright", legend = c("PCWARM","WARM"), lty = 1, 
       col = c("red","black"))


#Plotting the LTM
plot(1:predict_ahead, ltm_WARM/(ncol(training_set)),
     xlab = "Prediction Horizon", 
     ylab = "Fraction of Predictions - Correct",
     main = "WARM and PCWARM against LTM",
     ylim = c(0,1), 
     type='l'
     )
lines(1:predict_ahead, LTM_PCWARM/(ncol(training_set)),
      col = 'red')
legend("topright", legend = c("PCWARM","WARM"), lty = 1, 
       col = c("red","black"))
```
