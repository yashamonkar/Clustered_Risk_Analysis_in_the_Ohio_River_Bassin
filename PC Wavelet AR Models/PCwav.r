 PCwav=function(x,npcs,coords,name,yr1){
...
pcs=princomp(covmat=cx)
pcs$scores=x%*%pcs$loadings
...
for(i in 1:npcs){
  ...
  nam1=paste("PC",as.character(i),name)
  nam2=paste("Wavelet -PC",as.character(i))
  par(mfrow=c(2,2)); plot(yr1:yr2,p,xlab="Year",ylab=name,main=nam1)
  ...
  map('state');text(-113,28,"PC Loadings",cex=1.3)
  ju=pcs$loadings[,i];points(coords$Lon,coords$Lat,pch=19,cex=2,col=color.scale(ju,c(1,0.5,0),c(0,0.5,0),c(0,0,1),color.spec="rgb"))
