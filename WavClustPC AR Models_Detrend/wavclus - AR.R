...
#Converting to log. 
#Since we use PCA, which assumes multivariate normality, make the data more normal
#by means of taking its log helps a lot. 
...
#Plotting the PC's and detrending them.
pdf("plots/Detrended PCs.pdf")
par(mfrow=c(2,2));par(mar = c(4, 3, 3, 1))
detrended_PC <- matrix(NA, ncol = nclus, nrow = nrow(training_set))
trend_coeff <- matrix(NA, ncol = 2, nrow = nclus)
for(i in 1:nclus) {
  tpd=training_set[,cls[[i]]]
  pcw=prcomp(tpd, scale = TRUE)
  p=pcw$x[,1]
  ...
  plot(yr1:yr2,p, main = paste0("PC 1 of Cluster ", i),type='l',xlab = "Year")
  abline(fit,col='blue')
  lines(lowess(yr1:yr2,p,f=1/9),lwd=2,col="red")
  detrended_PC[,i] <- p-trend
  plot(yr1:yr2,detrended_PC[,i], main = paste0("Detrended PC 1 of Cluster ", i),type='l',xlab = "Year")
...
for(i in 1:nclus) {
  ...
  if(rapply(temp_list, length) == 0) {
    ...
    #Getting the predictions
    plot(yr1:yr2,p_org, type='l', main = paste0("Predictions for Cluster ", i, " PC 1")
